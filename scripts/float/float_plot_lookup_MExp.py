...
for i, dataset in enumerate(datasets):
    ...
    ax.plot(linear_double_data['size_in_MiB'], linear_double_data['lookup_time']/linear_double_data['n_samples'], label='pRMI', alpha=0.7, marker='o')
    
    # linear_regression_float 데이터
    linear_float_data = dataset_data[dataset_data['layer2'] == 'SIMD_float']
    ax.plot(linear_float_data['size_in_MiB'], linear_float_data['lookup_time']/linear_float_data['n_samples'], label='pRMI w/float', alpha=0.7, marker='o')
...
# PDF 파일로 저장
plt.savefig(output_dir + 'float_simd_lookup_MExp.svg')