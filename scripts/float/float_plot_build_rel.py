...
data = data.groupby(['dataset','layer1','layer2','n_models','size_in_bytes']).mean().reset_index()
...
for i, dataset in enumerate(datasets):
    ...
    avg_data1 = linear_data.groupby('n_models')['build_time'].mean()
    ...
    avg_data2 = linear_double_data.groupby('n_models')['build_time'].mean()
    
    # linear_regression_welford 데이터
    linear_float_data = dataset_data[dataset_data['layer2'] == 'SIMD_float']
    linear_float_data.reset_index(inplace=True, drop=True)
    avg_data3 = linear_float_data.groupby('n_models')['build_time'].mean()
    ...
    ax.plot(linear_data['size_in_MiB'], avg_data1/avg_data3, marker='o', color = "red")
...
# PDF 파일로 저장
plt.savefig(output_dir + 'float_simd_build_rel.svg')