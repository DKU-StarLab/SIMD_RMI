...
run() {
    ...
    timeout ${TIMEOUT} ${BIN} ${DATA_FILE} ${L1} ${L2} ${N_MODELS} ${BOUND} ${SEARCH} ${PARAMS} ${SWITCH} >> ${FILE_RESULTS}
}
...
# Write csv header
echo "dataset,n_keys,layer1,layer2,n_models,switch_n,bounds,search,size_in_bytes,rep,n_samples,lookup_latency,predict_latency,correct_latency,lookup_accu,predict_miss_ratio" > ${FILE_RESULTS} # Write csv header
    ...
    do
        ...
        do
            ...
            do
                ...
                run ${dataset} ${l1} ${l2} ${n_models} labs binary ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs binary_branchless ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs model_biased_binary_branchless ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs linear ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs linear_simd ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs model_biased_linear_simd ${switch}
                run ${dataset} ${l1} ${l2} ${n_models} labs exponential ${switch}
